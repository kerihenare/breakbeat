...
- Rendering: a `ViewRenderer` provider wrapping a Nunjucks `Environment` (autoescape on, `FileSystemLoader` at `process.cwd()/views`, so it works under both `pnpm dev` and `node dist`) renders templates to strings (no `setViewEngine`). Autoescape is the XSS guarantee for untrusted Result titles; `| safe` is the only (greppable) opt-out.
- Templates: `layout.njk`, `home.njk` (form + recent jobs), `job.njk` (full page wrapping `_job_live`), partials `_job_live.njk` (pane + SSE connector), `_job_pane.njk` (header: identified-as, status, window, count triad; grouped results; collapsed excluded `<details>`), `_result_group.njk`, `_result_row.njk`, `_recent_row.njk`, `404.njk`. Fixed Content-Type section order by editorial weight (News → Trade → Press → Podcast → Blog → Newsletter → Social → Other → Unclassified). Minimal styling now; Slice 8 applies the Clipping Desk brand.
